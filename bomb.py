...
class Bomb():
	def __init__(self, obj): #initialises the bomb onject
		self.planted =0
		self.x = 0
		self.y = 0
		self.timer = 3
	...
	def print_bomb(self, obj, a, b): #prints the bomb on the Matrix
		# if(obj.Matrix[a][b] == '['):
		for i in range (4):
			obj.Matrix[a][i + b] = colored(self.shape[i], 'cyan')
			obj.Matrix[a + 1][i + b] = colored(self.shape[i + 4], 'cyan')

	def blast(self, obj,enemy,player): #does the required functionality of the blast
		print("blast")
		self.planted = 0
		self.timer = 3
		for i in range (5):
